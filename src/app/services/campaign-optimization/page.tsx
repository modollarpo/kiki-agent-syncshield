import React from 'react';

export default function CampaignOptimizationPage() {
  return (
    <section className="max-w-4xl mx-auto py-12 px-6">
      <h1 className="text-3xl font-bold text-cyan-400 mb-4">Campaign Optimization</h1>
      <p className="text-zinc-400 mb-8">Deep-dive into KIKI's cross-platform bid orchestration and performance maximization logic.</p>
      <button className="mt-8 bg-white text-black font-bold px-5 py-2 rounded shadow-md hover:bg-cyan-400 hover:text-zinc-950 transition" onClick={() => window.location.href='/onboarding'}>
        Hire KIKI
      </button>
    </section>
  );
}
